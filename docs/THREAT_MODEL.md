...
| Gift-card product/package substitution | Exact Permit product ID is fetched; search is never auto-selected; the acquire Authorization binds denomination kind, exact package ID when fixed, face value, currency, and quantity one. Product facts are re-fetched before Send; a changed package ID fails closed. Invoice orders must match product and face value. |
| Undocumented Bitrefill catalog price units | `packages[].price` and `price_rate` are ignored. They are never treated as fiat minor units, mapped to `maxPurchasePriceMinor`, or used to infer FX. The exact Wavelength principal, fee, and total outflow are independently checked against the child `value.transfer` Permit before Send. |
