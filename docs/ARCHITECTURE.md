...
Permit evaluation is a pure function. Given a validated Permit v2, a validated ResolvedAction, evaluation timestamp, ledger-derived usage, and optional parent Authorization, it returns `ALLOW`, `DENY`, or `INDETERMINATE` with every applicable reason in stable order. It has no database write, clock side effect, network, or payment effect. Limits use integer fiat minor units and integer satoshis as independent dimensions. The engine does not convert fiat to bitcoin. The generic instrument domain retains optional `purchasePrice` / `maxPurchasePriceMinor` semantics for workflows with a safely sourced fiat price; Chunk 07 does not source those values from Bitrefill catalog `packages[].price` or `price_rate`.

For a Bitrefill gift card, the acquire Authorization binds provider, exact product, denomination kind, exact package ID when fixed, currency, face value, and quantity one. Its child `value.transfer` binds Wavelength's exact principal, fee, total outflow, payment hash, and prepared-operation digest. The transfer grant's satoshi limits are the final hard economic outflow bound and are revalidated before Send.
